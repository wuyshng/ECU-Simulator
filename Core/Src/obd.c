...
void ecu_display_LCD(uint8_t numberPID)
{
	...
	case 0: // Coolant Temp
		...
		curPIDValue = ecuValue.engine_coolant_temp;
		break;
	case 1: // Engine speed
...
void ecu_send_respond(void)
{
	...
	if (RxData[1] == MODE3)
	{
		...
		else
		{
			...
			TxData[4] = 0x01;
			TxData[5] = 0xC1; // Lost communication with head up display
			TxData[6] = 0x58;
			HAL_CAN_AddTxMessage(&hcan, &TxHeader, TxData, &TxMailbox);
		}
	}
